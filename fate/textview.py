...
class IntervalMapping:
    ...
    """
    Defines a mapping and its inverse that maps interval positions to interval positions across a
    series of substitutions in the text. The mapping is not bijective, injective or surjective,
    so the inverse is defined explicitly. Both the mapping and the inverse are proper
    functions.

    Since text operations, i.e. interval substitutions, are invertible, the inverse of an
    IntervalMapping is again an IntervalMapping.

    This mapping is meant to be a relatively dumb container. It contains logic to interpolate
    positions in between the interval boundaries that are given, but it can't for instance deal
    with overlapping intervals and it knows nothing about texts.
    The intervals must also be given in order, so we only support an append method.
    """

    def __init__(self, offset: int, _noffset: int=0, _inverse=None):
        """
        Offset is the position where the mapping starts in the original text.
        """
        assert offset >= 0
        self.offset = offset
        assert _noffset >= 0
        self.noffset = _noffset

        self._intervals = []
        self._cache = []

        self.inverse = _inverse or IntervalMapping(_noffset, offset, _inverse=self)

    def append(self, interval, image, _add_to_inverse=True):
        """
        Append an interval to the mapping.
        """
        min_beg = self._intervals[-1][1] if self._intervals else self.offset
        assert interval[0] >= min_beg

        self._intervals.append((interval, image))

        if _add_to_inverse:
            self.inverse.append(image, interval, _add_to_inverse=False)

    def __getitem__(self, item: (Interval, int)):
        """
        Apply the mapping to an interval or a interval position.

        :item: interval position or interval
        :returns: interval position or interval
        """
        # TODO: give neat out of bound errors
        if isinstance(item, Interval):
            beg, end = item
            return Interval(self._cache[self.offset + beg], self._cache[self.offset + end])
        elif isinstance(item, int):
            return self._cache[self.offset + item]
        else:
            raise TypeError

    def build_mapping(self, size: int, _build_inverse=True):
        """
        Build the mapping from the intervals.
        The size is the size of the mapping on the original interval side.
        """
        min_size = self._intervals[-1][1] if self._intervals else self.offset
        assert size > min_size

        pos = self.offset
        npos = self.noffset
        for (beg, end), (nbeg, nend) in self._intervals:
            # FIXME: deal correctly with adjacent intervals

            # Add non substituted positions
            for i in range(pos, beg):
                self._cache.append(npos)
                assert self._cache[i] == npos
                npos += 1
            pos = beg

            assert nbeg == npos
            for i in range(beg, end):
                # All positions within the interval (beg, end) map to nbeg,
                # except end, which maps to nend.
                self._cache.append(nbeg)
                assert self._cache[pos] == nbeg
                pos += 1

            self._cache.append(nend)
            pos += 1
            npos += nend - nbeg
            assert npos == nend
        ...
        if _build_inverse:
            self.inverse._build_mapping(inverse_size, _build_inverse=False)
